...
class Dimension {
	...
	constructor(name, tokenConfig) {
		...
		if (tokenConfig.step.type === 'exponential') {
			...
			if (typeof tokenConfig.step.exponent !== 'number' || tokenConfig.step.exponent % 1 !== 0) {
				throw new XError(XError.INVALID_ARGUMENT, 'exponent must be an integer');
			}
			...
			for (let i = tokenConfig.step.stepNum - 1; i >= 0; i--) {
				this.steps.push(
					roundToFiveSignificantDigits(tokenConfig.step.base * Math.pow(tokenConfig.step.exponent, i)));
			}
