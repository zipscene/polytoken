...
function getRayLineSegmentIntersectState([ point, point2 ], [ lineBegin, lineEnd ]) {
	...
	let [ beginX, beginY ] = lineBegin;
	let [ endX, endY ] = lineEnd;
	let raySlope = roundToFiveSignificantDigits((y2 - y) / (x2 - x));
	let lineSlope = roundToFiveSignificantDigits((endY - beginY) / (endX - beginX));
	if (isPointInLine(point, [ lineBegin, lineEnd ]) && isPointInLine(point2, [ lineBegin, lineEnd ])) {
		if ((roundToFiveSignificantDigits((x - x2) * (x - beginX)) < 0
			&& roundToFiveSignificantDigits((x - x2 * (x - endX)) < 0))
			|| (roundToFiveSignificantDigits((y - y2) * (y - beginY)) < 0
			&& roundToFiveSignificantDigits((y - y2 * (y - endY)) < 0))
		) {
			return UNINTERSECT;
		} else {
			return OVERLAP;
		...
		if (intersectX < x) return UNINTERSECT;
		if (roundToFiveSignificantDigits((intersectX - beginX) * (intersectX - endX)) < 0
			|| roundToFiveSignificantDigits((intersectY - beginY) * (intersectY - endY)) < 0) {
			return INTERSECT;
		} else if (roundToFiveSignificantDigits((intersectX - beginX) * (intersectX - endX)) > 0
			|| roundToFiveSignificantDigits((intersectY - beginX) * (intersectY - endY)) > 0) {
			return UNINTERSECT;
		} else {
			return TANGENT;
		}
