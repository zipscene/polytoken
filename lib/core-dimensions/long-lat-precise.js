...
class LongLatPrecise extends Dimension {
				...
				for (let j = 0; j < jMax; j++) {
					...
					if (state === 'inside') {
						...
						if (isInsideInterior) {
							...
							// Push token that's inside exterior and outside interiors into `tokens`. Also convert
							// longitude that exceeds meridian
	...
	_getRangeTokens(range, intersection, stepIndex, tokens) {
		...
		let newIntersection = [];
		for (let [ x, y ] of intersection) {
			for (let i = 0; i < this.tokenConfig.step.multiplier; i++) {
				let currentX = utils.roundToFiveSignificantDigits(x + i * step);
				for (let j = 0; j < this.tokenConfig.step.multiplier; j++) {
