...
class NumericDimension extends Dimension {
	...
	validateRange(range) {
		if (!Array.isArray(range)) {
			throw new XError(XError.INVALID_RANGE, 'Range of numberic dimensions should be an array');
		}
		if (range.length !== 2) {
			throw new XError(XError.INVALID_RANGE, 'Range of numberic dimensions should have length of two');
		}
		if (typeof range[0] !== 'number' || typeof range[1] !== 'number') {
			throw new XError(XError.INVALID_RANGE, 'Range of numberic dimensions should consist of numbers');
		}
		if (range[1] <= range[0]) {
			throw new XError(XError.INVALID_RANGE, 'Range upper bound must be greater than range lower bound');
		}
		return true;
	}
	...
	_getRangeTokens(min, max, curStepIndex, tokens = []) {
		...
		if (curStepIndex >= steps.length) {
			...
			if (length > 0) {
				tokens.push(`${Math.floor(min / step) * step}^${step}`);
			}
			return tokens;
		}
		...
		// Go to the next (smaller) step if current step is greater than range or
		// current step is greater than upper range minus the ceil of lower bound
		// in the scale of current step, i.e. 2.4 given step 2 will have a ceil of
		// 4. We're doing it this way because tokens are defined to begin with an
		// interger number of steps
		if (step > length || length < (step - min % step) % step + step) {
			return this._getRangeTokens(min, max, curStepIndex + 1);
		}
		let pivot = min % step === 0 ? min : min + (step - min % step);
